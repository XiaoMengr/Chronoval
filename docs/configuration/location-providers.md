...
By default, no configuration is required. Chronoval uses OpenStreetMap Nominatim as the default location provider.
...
Chronoval supports two location providers. You can choose the appropriate one based on your needs.

## Nominatim

Nominatim is a free reverse geocoding service provided by OpenStreetMap. By default, Chronoval uses Nominatim as the location provider.
...
When you configure the `NUXT_MAPBOX_ACCESS_TOKEN` environment variable, Chronoval will automatically use Mapbox Search API as the location provider.
