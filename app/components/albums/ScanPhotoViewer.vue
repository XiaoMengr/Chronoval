  ...
  <Teleport to="body">
    ...
    <AnimatePresence>
      <motion.div
        ...
        class="fixed inset-0 z-[120] bg-[#0a0a0e]/[0.97]"
        @click="emit('close')"
      />
    </AnimatePresence>

    <!-- ThumbHash 常驻背景（与首页查看器一致，切换仅换 hash 数据、不做整屏闪变） -->
    <AnimatePresence>
      <motion.div
        v-if="props.isOpen"
        :initial="{ opacity: 1 }"
    ...
    <AnimatePresence>
      <motion.div
        ...
        :exit="{ opacity: 0 }"
        :transition="{ duration: 0.3 }"
        class="fixed inset-0 z-[122] flex items-center justify-center"
